...
namespace Testing.Validation
{
    public class PostDTOValidationTest
    {
        ...
        public void Should_Pass_With_Max_Values()
        {
            ...
            _postDTO.TimePosted = DateTime.Now.AddSeconds(-59);
            _postDTO.Comments = new List<CommentDTO> { new() };
            _postDTO.Likes = new List<LikeDTO> { new() };
            _postDTO.Pictures = new List<PictureDTO> { new() };
            var result = ValidateModel(_postDTO);
            Assert.True(result);
        }
        ...
        [Fact]
        public void Should_Fail_TimePosted_Min()
        {
            _postDTO.TimePosted = DateTime.Now.AddSeconds(1);
            var result = ValidateModel(_postDTO);
            Assert.False(result);
        }

        [Fact]
        public void Should_Fail_TimePosted_Max()
        {
            _postDTO.TimePosted = DateTime.Now.AddSeconds(-61);
            var result = ValidateModel(_postDTO);
            Assert.False(result);
        }
